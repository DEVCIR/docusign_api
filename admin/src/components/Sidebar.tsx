...
const Sidebar = ({ activePage = "dashboard", activeSubPage = "" }: SidebarProps) => {
  ...
  const [expandedItems, setExpandedItems] = useState<Set<string>>(() => {
    ...
    if (activePage === "user/list" || activeSubPage?.startsWith("user")) {
      items.add("user")
      items.add("user/list")
    }
    if (activePage === "templates" || activeSubPage?.startsWith("envelope-")) {
      items.add("templates")
      items.add("envelope-templates")
    }
    if (activePage === "folders" || activeSubPage?.startsWith("folder-")) {
      items.add("folders")
    }
    if (activePage === "agreements") {
