  ...
  const renderBoxes = ( boxType ) =>
  {
    ...
    return boxes.map( ( box, index ) =>
    {
      ...
      const boxStyle = {
        ...
        // padding: '2px',
        transition: 'transform 0.3s cubic-bezier(0.25, 0.1, 0.25, 1), opacity 0.3s cubic-bezier(0.25, 0.1, 0.25, 1)', // Added transition for animation
        opacity: isDraggingThisBox ? 0.9 : 1,
        transform: isFocused ? 'scale(1.05)' : 'scale(1)', // Scale effect on focus and unfocus
        userSelect: 'none',
        ...
        >
          {isFocused && (
            <>
              ...
              <CTooltip className='inline-flex' content="Required" placement='ne'>
                <span>
                  <input
                    type="checkbox"
